...
import { getPlayLists, getLayouts, removePlayList, removeLayout } from './PlayListFunctions'
...
const Layout = props => {
  ...
  return(
    <div className={"flex_container_row header_back pad5 marginRight " + props.bgc} >
        ...
        <div className="btn deleteBtn visible" 
          ...
          onClick={props.removeLayoutStart.bind(this)}>
              <i aria-hidden="true" 
                ...
                id={"i-"+props.id}
                onClick={props.removeLayoutStart.bind(this)}></i>
        </div>
...
const Layouts = props => {
  return (
    props.layoutsArr.map(layout => <Layout
      key={uuid()} 
...
const PlayList = props => {
  return(
    <div className={"flex_container_row flex_container_row no-shrink pad5 marginRight " + props.bgc} >
...
const Playlists = props => {
  return (
    props.playListsArr.map(playlist => <PlayList  
      key={uuid()}  
      id={playlist.id}
      name={playlist.name}
...
export const PlayLists = () => {
  ...
  const removePlayListStart = e => {
    ...
    if(window.confirm("Are you sure you want to delete this?")){
        ...
        removePlayList(id,thetoken).then( () => {
          setPlayListsArr([...doStripe(playListsArr.filter(alist => parseInt(alist.id) !== parseInt(id)))])
        })
  ...
  const removeLayoutStart = e => {
    ...
    if(window.confirm("Are you sure you want to delete this?")){
        ...
        removeLayout(id,thetoken).then( () => {
          setLayoutsArr(doStripe(layoutsArr.filter(alayout => parseInt(alayout.id) !== parseInt(id))))
        })
    } else { return false }        
  ...
  return (
    <div id="main" >
      ...
      <Playlists playListsArr={playListsArr} 
