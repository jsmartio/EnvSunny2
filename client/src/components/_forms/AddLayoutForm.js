    ...
    useEffect(() => { 
        if(props.thetoken !== 'Token not Set'){
            ...
            dbx.filesListFolder({path: ''})
                .then(res => {
                    getDonorCats(props.thetoken).then(res2 => {
                        if(res !== false && res2 !== false) {
                            setDonorCatsArr(res2)
                            setMediaArr(res.entries)    // set state for media Array (dropbox)
                            setMediaArr([...res.entries])
                            setAlertMsg('Media has loaded from dropbox')
                        } else {
        ...
        <form noValidate onSubmit={onSubmit}>
            ...
            <FormGroup>
                ...
                <Input type="select" onChange={event => setAsset(event.target.value)}   
                    ...
                    {mediaArr.map(media => <option key={media.name} 
