...
import { Alert, Button,  FormGroup, Input, Label, Spinner, Tooltip } from 'reactstrap';
...
export const AddPlayListForm = (props) => {
    ...
    const [idp,setIdp] = useState(uuid()) 
    ...
    const [mediaArrp, setMediaArrp] = useState([])
    ...
    function onSubmit(e) {
        ...
        // do validation
        var errMsg = ''
        //if(PLname === undefined) errMsg += "\n\tName missing"
        //if(author === undefined) errMsg += "\n\tAuthor missing"
        ...
        if(errMsg !== ''){
            ...
            addPlayLists(newPlaylist,props.thetoken).then( res => {
                console.log(res)
                if(res !== undefined){
    ...
    useEffect(() => {
        if(props.thetoken !== 'Token not Set'){
            var dbx = new Dropbox.Dropbox({ fetch: fetch,accessToken: o.global.dropBoxAccessToken });
            const tmp = []
            dbx.filesListFolder({path: ''})
                .then(res => {
                    
                    //setMediaArrp(res.entries)    // set state for media Array (dropbox)
                    setMediaArrp([...res.entries])
                    ...
                    setMovieArrp(tmp)           
                    setAlertMsgp('Media has loaded from dropbox')
                    setSpinClassp('hid')
                })
        }
      }, [props.thetoken]);
    ...
    return (
        <form noValidate onSubmit={onSubmit}>
            <Alert color="primary" id="errmsg" className={alertClassp}> 
                <Spinner type="grow" color="primary" className={spinClassp}/><pre>{alertMsgp}</pre>
            </Alert>
            ...
            <FormGroup>
                ...
                <Input type="select" onChange={event => setPlacementp(event.target.value)}   
                    id="placementp" 
            ...
            <FormGroup  style={grayFormGroup}>
                <Label for="endtime">Play Order &nbsp; 
                <span className="rounded1" id="tool1">?</span>
                    <Tooltip placement="right" isOpen={tooltipOpen} target="tool1" toggle={toggletool}>
                        (A Number) Which determines the positional order on the playlist
                    </Tooltip>
                </Label>
                    <Input type="text" 
                        id="playOrder" 
            ...
            <FormGroup>
                <Label for="endtime">Duration &nbsp;
                    <span className="rounded1" id="tool2">?</span>
                    <Tooltip placement="right" isOpen={tooltipOpen} target="tool2" toggle={toggletool}>
            ...
            <FormGroup style={grayFormGroup}> &nbsp; 
                <Label for="endtime">Transition Duration &nbsp; 
                    <span className="rounded1" id="tool3">?</span>
                    <Tooltip placement="right" isOpen={tooltipOpen} target="tool3" toggle={toggletool}>
            ...
            <FormGroup  >
                <Label for="endtime">Fade In &nbsp; 
                    <span className="rounded1" id="tool4">?</span>
                    <Tooltip placement="right" isOpen={tooltipOpen} target="tool4" toggle={toggletool}>
            ...
            <FormGroup  >
                <Label for="endtime">Type &nbsp; 
                    <span className="rounded1" id="tool5">?</span>
                    <Tooltip placement="right" isOpen={tooltipOpen} target="tool5" toggle={toggletool}>
                        A dictionary description of the transition type
                    </Tooltip>
                </Label>
                    <Input type="select" onChange={event => setTypep(event.target.value)}      
                            id="typep"
                            placeholder="Type" 
                            value={typep}
                        >
            ...
            <FormGroup style={grayFormGroup}>
                ...
                <Input type="select" onChange={event => setAssetp(event.target.value)}
                    id="assetp" 
                    placeholder="asset" 
                    value={assetp}>
                        <option value='None Specified'>NA</option>
                    {mediaArrp.map(media => <option key={media.name} 
            ...
            <FormGroup style={grayFormGroup}>
                ...
                <Input type="select" onChange={event => setLayoutp(event.target.value)}   
                    name="layoutp" 
                    id="layoutp" 
                    placeholder="layout" 
                    value={layoutp}>
            ...
            <FormGroup>
                ...
                <Input type="select" onChange={event => setBgMoviep(event.target.value)}   
                    id="bgMoviep" 
                    placeholder="bgMovie" 
                    value={bgMoviep}>
                        <option value='None Specified'>NA</option>
                    {movieArrp.map(movie => <option key={movie} 
            ...
            <FormGroup style={grayFormGroup}>
                <Label for="solop">Solo &nbsp; 
                    <span className="rounded1" id="tool6">?</span>
                    <Tooltip placement="right" isOpen={tooltipOpen} target="tool6" toggle={toggletool}>
                        (true or false) is this a solo playlist.  If so all others will halt during the process
                    </Tooltip>
                </Label>
                <Input type="select" onChange={event => setSolop(event.target.value)}
                        id="solop" 
                        placeholder="solo" 
                        value={solop}>
